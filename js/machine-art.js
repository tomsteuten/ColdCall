 ...
 *   'open'    — mid-teardown: lids/panels off, internals exposed
 ...
 *   'log'     — cabinet closed, controller awake with stored alarms
...
// Each machine has three base states (fault/open/working) plus the four
// interaction states (log/probe/leads/ajar) that labelled diagnostic controls
// swap to — all scale-/camera-matched to the fault render so the swap never
// jumps (see assets/generated/PROMPTS.md for how the interaction set was made).
const RENDER_STATES = ['fault', 'open', 'working', 'log', 'probe', 'leads', 'ajar'];
 ...
 * one art lane now that every machine has all seven states; the SVG
 ...
 * @param {'fault'|'open'|'working'|'log'|'probe'|'leads'|'ajar'} state
 * @returns {string|null}
 */
