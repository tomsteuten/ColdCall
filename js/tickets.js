 ...
 * No tier gating yet — player.tierUnlocked is cosmetic until reputation gating
 ...
 * Faults whose tier has no client are never picked — a ticket needs a caller.
 ...
 */
export function pickTicket(faults, clients, next) {
  ...
  if (candidates.length === 0) throw new Error('No fault has a client of the same tier');
