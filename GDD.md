...
3. **Workshop** — refurbish ruined machines bought cheap, sell refurbed (idle money sink/converter).
   - **Decision of record (v1.0, as implemented):** the workshop is an *active* converter, not an idle one. You buy a damaged machine (`config/balance.js` `WORKSHOP`, gated by `tierRequired` to your unlocked client tier), diagnose+repair it through the **normal diagnosis minigame**, then sell the refurb for `sellPrice × founderBonus`. The repair job itself pays $0 and no reputation — the profit is the buy→sell spread. It deliberately gives no rep and carries no callback risk, so it's a sink/converter that doesn't compete with reputation progression. Per-job dollar value is in the same band as a fresh ticket; the tier-2 spread sits slightly above a fresh tier-2 ticket's best net and is flagged in `balance.js` for the playtest pass (it never touches idle income, so the active>idle rule §3.1 is untouched).
...
- **Decision of record (v1.0, as implemented):** prestige unlocks at `PRESTIGE.lifetimeEarningsThreshold` ($250k, GDD §6). Selling adds `reputation × PRESTIGE.bonusPerReputation` (0.01 = +1% per rep held at sell time) to the permanent `founderBonus`, which accumulates across prestiges and resets everything else (cash, rep, tier, tools, van, techs, routes, jobs, workshop, offline carry). The Founder Bonus multiplies **active** earnings *and* reputation gain — so each post-prestige run reaches the next tier faster — but is **not** applied to idle tech income (`idle.js`), so the active>idle rule (§3.1) is preserved. (The bonus derives from *current* reputation at sell time, not a separate lifetime-reputation total; because it accumulates each prestige the effect matches the "from lifetime reputation" intent.)
