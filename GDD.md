...
3. **Workshop** — refurbish ruined machines bought cheap, sell refurbed (idle money sink/converter).
   ...
   - **Decision of record (2026-07-28) — workshop production line:** the mature Operations Board exposes **Receiving → Repair → Ready** as three physical bays. Capacities live in `balance.js` (2 receiving / 1 repair / 2 ready). Buying stops when Receiving is full; a new repair cannot start while another job is active or Ready has no free slot; selling clears Ready. The existing active workshop diagnosis is the Repair bay, while `broken` and `repaired` machine states derive the other two bays — no new save fields or migration are needed. An imported old save above capacity keeps every machine and simply cannot add more until the backlog is cleared. Margins and the active-play economy are unchanged.
